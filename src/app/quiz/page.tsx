...
export default function PdfQuizGenerator() {
  ...
  const [questionsBlob, setQuestionsBlob] = useState<Blob | null>(null);
  const instructionData = toolData["pdf-quiz"];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setQuestions(data.result || "No questions generated");
        const blob = new Blob([data.result], { type: "text/plain" });
        setQuestionsBlob(blob);
      }
    } catch (err: any) {
  ...
  const handleShare = () => {
    if (!questionsBlob) {
      alert("Please generate questions first before sharing");
      return;
    }
