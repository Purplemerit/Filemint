...
export default function PdfToWordPage() {
  ...
  return (
    <div>
          ...
          >
            {isConverted ? (
              <div style={{
                ...
                <h2 style={{ fontSize: "1.75rem", color: "#333", margin: 0, fontWeight: "600" }}>
                  Conversion Complete!
                </h2>
                <button
          ...
          <div style={{ marginTop: "3rem", fontFamily: 'Georgia, serif' }}>
            ...
            <div style={{
              ...
              <p style={{ color: "#1e3a8a", lineHeight: "1.6", margin: 0 }}>
                High-fidelity PDF to Word conversion requires advanced reconstruction of paragraph flows, table structures, and font mappings. Our engine ensures that every element remains editable while preserving the original visual intent.
              </p>
            </div>
          </div>
        </div>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000 }} onClick={() => setShowUrlModal(false)}>
            <div style={{ backgroundColor: "white", padding: "2rem", borderRadius: "10px", width: "90%", maxWidth: "500px" }} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                ...
                <button onClick={handleUrlSubmit} disabled={isUploading} style={{ padding: "0.5rem 1rem", border: "none", borderRadius: "6px", backgroundColor: "#D879FD", color: "white", cursor: isUploading ? "not-allowed" : "pointer", opacity: isUploading ? 0.7 : 1 }}>{isUploading ? "Loading..." : "Add PDF"}</button>
              </div>
            </div>
          </div>
        )
      ...
      <ShareModal isOpen={showShareModal} onClose={() => setShowShareModal(false)} fileBlob={zipBlob} fileName={files.length > 1 ? "converted_files.zip" : (files[0]?.file.name.replace(".pdf", ".docx") || "document.docx")} onDownload={handleDownload} />
