...
export default function PptToPdfPage() {
  ...
  const instructionData = toolData["ppt-to-pdf"];
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `converted_${file?.name?.replace(/\.[^/.]+$/, "") || "presentation"}.pdf`;
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      const fileName = urlInput.split("/").pop() || "downloaded.pptx";
      if (!fileName.endsWith(".ppt") && !fileName.endsWith(".pptx")) {
        alert("URL must point to a PPT or PPTX file");
        return;
      }
      ...
      alert("Failed to fetch PowerPoint from URL");
    } finally {
  ...
  const handleFromClipboard = async () => {
    try {
      ...
      for (const item of clipboardItems) {
        const pptTypes = item.types.filter(type =>
          type.includes("presentation") || type.includes("powerpoint")
        );
        if (pptTypes.length > 0) {
          const blob = await item.getType(pptTypes[0]);
          const file = new File([blob], "clipboard.pptx", { type: pptTypes[0] });
          setFile(file);
          setError(null);
          break;
        }
      ...
      alert("No PowerPoint file found in clipboard or clipboard access denied");
    }
    setIsDropdownOpen(false);
  };
    ...
    <div>
            ...
            {isConverted ? (
              ...
              <div style={{
                ...
                <p style={{ color: "#666", textAlign: "center", maxWidth: "400px" }}>
                  Your PowerPoint presentation has been converted to PDF. Download your file below.
                </p>
              ...
              <div>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <img src="./ppt.png" alt="PPT Icon" style={{ width: "40px", height: "50px", marginBottom: "0.5rem" }} />
                    <span style={{
                      fontSize: "0.65rem",
                      color: "#666",
                      maxWidth: "100px",
      ...
      {showUrlModal && (
        ...
        >
          ...
          >
            <h3 style={{ marginBottom: "1rem" }}>Paste PowerPoint URL</h3>
            <input
              type="url"
              value={urlInput}
            ...
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
              ...
              >
                {isUploading ? "Loading..." : "Add PowerPoint"}
              </button>
            </div>
          </div>
        </div>
